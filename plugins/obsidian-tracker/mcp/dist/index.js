...
async function requireVault() {
    ...
    if (!vaultPath) {
        throw new Error("Obsidian Tracker not initialized. Please run initVault first with your Obsidian vault path.");
    }
    const isValid = await validateVaultPath(vaultPath);
    if (!isValid) {
        throw new Error(`Vault path "${vaultPath}" does not exist or is not a directory. Please run initVault with a valid path.`);
    }
    return vaultPath;
}
/**
