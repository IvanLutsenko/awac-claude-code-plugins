...
### ⚠️ КРИТИЧЕСКАЯ ПРОВЕРКА перед тестированием КАЖДОГО метода

**Для каждого public метода проверь три вещи:**

```
□ Возвращает значение? (не void)
  ✅ fun getKey(): String  → ТЕСТИРОВАТЬ
  ❌ fun logEvent(name: String)  → ПРОПУСТИТЬ

□ Может быть замокирована его зависимость?
  ✅ api.getStatus()  → можно замокировать → ТЕСТИРОВАТЬ
  ❌ Firebase.track()  → не мокируется → ПРОПУСТИТЬ

□ Есть логика которую можно асёртить?
  ✅ if (enabled) return true  → ТЕСТИРОВАТЬ
  ❌ receiver.method()  (просто forward) → ПРОПУСТИТЬ

Все YES? → создавай тест для этого метода
Хотя бы один NO? → пропусти этот метод, нечего асёртить
```
