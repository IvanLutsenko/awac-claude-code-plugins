...
3. If works → use MCP for all requests (Steps 1-5)
   Build console_url: "https://console.firebase.google.com/project/{PROJECT_ID}/crashlytics/app/android:{APP_ID}/issues/{ISSUE_ID}"
...
4. Get access_token from saved Firebase CLI credentials:
   ...
   (embedded in firebase-tools, this is an installed app OAuth flow)
...
If at Step 0 you already got project_id and app_id via CLI — request data directly:

```bash
# Get access token (if not already obtained at Step 0)
...
Parse JSON and extract: title, type, status, stack_traces, device_info, event_count.
**DO NOT call firebase-fetcher agent** — data already obtained.

**Option C: Manual fallback**

If neither MCP nor CLI API worked — generate Console URL and ask for manual input:
```
https://console.firebase.google.com/project/{PROJECT_ID}/crashlytics/app/android:{APP_ID}/issues/{ISSUE_ID}
```
